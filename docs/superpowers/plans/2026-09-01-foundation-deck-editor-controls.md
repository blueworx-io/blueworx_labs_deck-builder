...
Executed on 2026-09-01, on `add-deck-editor-controls` in `bluegroup_core_foundation` —
[PR #69](https://github.com/blueworx-io/bluegroup_core_foundation/pull/69), all checks green.

Five things the plan got wrong or did not foresee. Recorded here because plans 2 and 3
should build on what is true, not on what this one assumed.

1. **The foundation has no `CHANGELOG.md`,** and its CI does not ask for one — it is versioned
   by tags. Task 4's changelog step was skipped, and the readme carries the record instead.
2. **`_ds_bundle.js` and `_ds_manifest.json` are generated by Claude Design,** and the repo
   has no script to rebuild them. The inventory cards render from the bundle's global, so
   both new components had to be hand-transpiled into it, and `DataTable`'s changed body
   swapped in. Never re-sort those lists — insert beside the components from the same
   directory, or a two-line addition becomes a whole-file diff.
3. **`Store::castByKind()` carries its own closed list of array-valued kinds,** which the plan
   missed entirely. A gantt saved as a string, logged a PHP warning and lost the phases. Only
   the Playwright specs caught it — the PHPUnit tests of `Sanitise` all passed. The lesson is
   the general one: a new kind needs a spec that saves and reloads, not only a unit test.
4. **Task 3 Step 4's adherence check was meaningless as written.** `stray-admin-css` fires on
   every selector that is not a documented chrome override, so it fires on every design system
   rule by design — and the design system is exempt from it by path. Only the value rules
   (`raw-color`, `raw-size`, `raw-shadow`, `raw-font`) are worth running over a new block, and
   all three blocks pass them.
5. **The suite's timeout was too tight before this work and is now demonstrably so.** Several
   pre-existing specs pass alone and time out in a full run. Raised to 90s in
   `playwright.config.js`.

Two smaller corrections: the real APIs are `Schema::validate()` and `Validate::run()`, not the
`Schema::screen()` / `Validate::field()` the plan's test code invented; and a timeline's dates
are now formatted `en-GB` explicitly, because a test that passed locally failed on CI where
the runner's locale put the month first.

**Still open, and deliberately so:** "save this line item to the content library" and
insert-from-library on a sections list are plugin behaviour, not controls. They belong to the
Deck Builder plan.
